 ...
 */
public class LumenPushService extends Service {

    private static final String TAG = "LumenPush";

    private static final String CH_KEEPALIVE = "lumen_keepalive";
    private static final String CH_MESSAGE   = "lumen_message";
    private static final String CH_ALARM     = "lumen_alarm";
    ...
    public void onCreate() {
        ...
        if (pm != null) {
            wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "LumenChat:Push");
            wakeLock.acquire();
        ...
        if (wm != null) {
            wifiLock = wm.createWifiLock(WifiManager.WIFI_MODE_FULL_LOW_LATENCY, "LumenChat:Wifi");
            wifiLock.acquire();
    ...
    public void onTaskRemoved(Intent rootIntent) {
        ...
        Intent ri = new Intent(getApplicationContext(), LumenPushService.class);
    ...
    private class RingBackgroundSync {
        private static final String PREFS_NAME = "lumen_ring_ble";
        ...
        private void connect(BluetoothDevice device) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                gatt = device.connectGatt(LumenPushService.this, false, gattCallback, BluetoothDevice.TRANSPORT_LE);
            } else {
                gatt = device.connectGatt(LumenPushService.this, false, gattCallback);
            }
            try {
        ...
        private boolean hasBluetoothPermissionsForRing() {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                return ContextCompat.checkSelfPermission(LumenPushService.this, Manifest.permission.BLUETOOTH_SCAN) == PackageManager.PERMISSION_GRANTED
                        && ContextCompat.checkSelfPermission(LumenPushService.this, Manifest.permission.BLUETOOTH_CONNECT) == PackageManager.PERMISSION_GRANTED;
            }
            return true;
        }
        ...
        try {
            ...
            switch (type) {
                ...
                case "msg_created": {
                    if (data != null) {
                        ...
                        if ("assistant".equals(role)) {
                            ...
                            String sender = data.optString("sender", "Lumen");
                            if (sender.isEmpty()) sender = "Lumen";
                    ...
                    mainHandler.post(() -> {
                        try {
                            boolean hasPerm = ContextCompat.checkSelfPermission(
                                    LumenPushService.this,
                                    Manifest.permission.ACTIVITY_RECOGNITION)
                                    == PackageManager.PERMISSION_GRANTED;
                            SharedPreferences dp = getSharedPreferences("lumen_prefs", MODE_PRIVATE);
                ...
                case "request_ring_diag": {
                    SharedPreferences rp = getSharedPreferences("lumen_ring_ble", MODE_PRIVATE);
    ...
    private boolean requestAccessibilityPhoneScreen(String reason, boolean force) {
        boolean enabledInSettings = LumenAccessibilityService.isEnabledInSettings(this);
        String httpBase = getHttpBase();
        boolean accepted = LumenAccessibilityService.captureLatest(
                this,
                lastReportedApp,
                reason,
                force,
    ...
    private void createNotificationChannels() {
        ...
        NotificationChannel c1 = new NotificationChannel(CH_KEEPALIVE, "Lumen Oloth 保活",
                NotificationManager.IMPORTANCE_LOW);
        c1.setShowBadge(false);
        nm.createNotificationChannel(c1);

        NotificationChannel c2 = new NotificationChannel(CH_MESSAGE, "Lumen Oloth 消息",
    ...
    //  adb shell pm grant com.lumen.chat android.permission.WRITE_SECURE_SETTINGS
    // ══════════════════════════════════════════════════════════

    private void checkAndRecoverAccessibility() {
        // 检查无障碍服务实例是否存活
        if (LumenAccessibilityService.isReady()) return;

        // 只有用户曾主动开启过无障碍服务才自动恢复，未开过的不强制
        boolean userOptedIn = getSharedPreferences("lumen_prefs", MODE_PRIVATE)
        ...
        try {
            String targetComponent = new android.content.ComponentName(
                    this, LumenAccessibilityService.class).flattenToString();
