...
def display_name_for_sender(sender: str) -> str:
    ...
    return {"user": user_name, "assistant": ai_name, "lumen": ai_name, "connor": connor_name}.get(sender, sender)
...
async def build_surfacing_chatroom_memories(
    ...
    """
    构建 Connor 侧的 [背景记忆] 注入内容（对标 Lumen 的 build_surfacing_memories）。
    策略：
...
async def fetch_chatroom_source_details(memories: list[dict], keywords: list[str]) -> str:
    ...
    去重、按时间排序后返回（对标 Lumen 的 fetch_source_details）。
    """
...
async def digest_chatroom(room_id: str = None, model_key: str = None) -> dict:
    ...
    # 构建人设前缀（Connor 总结只带自己的人设 + 用户信息，不带 Lumen 人设）
    persona_block = ""
    ...
    for group in groups:
        ...
        for m in group:
            ...
            name = {"user": user_name, "lumen": ai_name, "connor": connor_name}.get(m["sender"], m["sender"])
...
async def build_lumen_group_context(
    room_id: str,
    ...
    *,
    ...
    """为 Lumen 在群聊中构建完整上下文（含系统能力、记忆召回、时间感知）。
    ...
    merged = await fetch_merged_timeline("lumen", context_limit, room_id=room_id)

    recent_for_digest = []
    for msg in merged[-6:]:
        sender = msg.get("sender", "user")
        role = "assistant" if sender in ("lumen", "assistant") else "user"
    ...
    # 4. 记忆召回（使用共享模块，Lumen 读主记忆库 + 聊天室记忆）
    ...
    # 6. 统一时间线（合并私聊 + 群聊消息）
    timeline_history = render_merged_timeline(merged, "lumen")
...
async def build_connor_group_context(
    ...
    *,
    ...
    # 3. 记忆召回（Connor 只读聊天室记忆，不读 Lumen 主记忆库）
    async def _chatroom_recall(query, keywords):
