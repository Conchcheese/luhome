...
def _next_idle_actor() -> str:
    ...
    if last == "connor":
        return "lumen"
    return random.choice(["lumen", "connor"])


def _record_idle_actor(actor: str):
    if actor not in ("lumen", "connor"):
        return
...
async def _lumen_model() -> str:
    target = manager.get_lumen_last_active()
    if target and target.startswith("chatroom:"):
        try:
            from chatroom import load_chatroom_config
            model = (load_chatroom_config().get("lumen_model") or "").strip()
            if model:
                return model
        except Exception:
            pass
...
async def _call_actor(actor: str, messages: list[dict]) -> str:
    ...
    return await _collect(stream_ai(messages, await _lumen_model(), {}))
...
async def _actor_context(actor: str, limit: int = 30) -> list[dict]:
    ...
    if actor == "lumen":
        ...
        if wb.get("user_persona"):
            ...
            messages.append({"role": "assistant", "content": "收到。"})
        timeline = await fetch_merged_timeline("lumen", limit, room_id=room_id)
        messages.extend(render_merged_timeline(timeline, "lumen"))
        return messages
...
async def _save_lumen_private_message(content: str) -> dict | None:
...
async def _save_private_message(actor: str, content: str) -> dict | None:
    if actor == "lumen":
        return await _save_lumen_private_message(content)
...
async def _run_role_chat(actor: str) -> dict:
    from routes.chatroom import _load_room_and_messages, _reply_lumen, _reply_connor, _save_msg

    room_id = await _latest_group_room_id()
    if not room_id:
        raise RuntimeError("没有可用的群聊房间")
    target = "connor" if actor == "lumen" else "lumen"
    actor_name = _actor_label(actor)
    ...
    if target == "lumen":
        await _reply_lumen(room_id, msgs, context_limit, message, await _lumen_model(), queue)
    else:
...
async def _home_dynamics_text(hours: int = 6, limit: int = 80) -> str:
    ...
    async with get_db() as db:
        ...
        for r in await cur.fetchall():
            items.append((r["created_at"], f"{_actor_label(r['sender'] or 'lumen')}送出了礼物：{_clip(r['message'], 120)}"))
        cur = await db.execute(
...
async def _run_cam_check(actor: str) -> dict:
    ...
    if actor == "lumen":
        ...
        if target and target.startswith("chatroom:"):
            ...
            await _chatroom_cam_check(room_id, "lumen", await _lumen_model(), delay=0)
        else:
            from camera import perform_cam_check
            conv_id, model = await _latest_conversation()
            if not conv_id:
                raise RuntimeError("没有可用的 Lumen 私聊会话")
