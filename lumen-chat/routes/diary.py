...
async def list_diaries(
    ...
    """分页获取日记，author 可选：user / lumen / connor。"""
...
async def synthesize_diary_tts(entry_id: str):
    ...
    async with lock:
        ...
        voice_key = "tts_lumen_voice" if author == "lumen" else "tts_connor_voice"
        voice = str(cfg.get(voice_key) or "").strip()
        if not voice:
            label = "Lumen" if author == "lumen" else cfg.get("connor_name", "Connor")
