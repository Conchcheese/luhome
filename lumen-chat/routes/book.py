...
async def get_chapter(book_id: str, ch_idx: int):
    async with get_db() as db:
        ...
        # 获取已有批注（区分 annotator）
        ann_rows = await db.execute("""
            SELECT segment_index, annotations, summary, updated_at, COALESCE(annotator, 'lumen') as annotator
        ...
        hl_rows = await db.execute("""
            SELECT id, selected_text, start_p, start_offset, end_p, end_offset,
                   ...
                   COALESCE(annotator, 'lumen') as annotator,
    ...
    # 解析批注（按 annotator 分组）
    lumen_annotations_map = {}  # p_idx -> annotation
    connor_annotations_map = {}
    lumen_summaries = []
    connor_summaries = []
    for ann in annotations_raw:
        annotator = ann.get('annotator', 'lumen')
        target_map = lumen_annotations_map if annotator == 'lumen' else connor_annotations_map
        target_summaries = lumen_summaries if annotator == 'lumen' else connor_summaries
        try:
    ...
    for hl in highlights_raw:
        highlights.append({
            ...
            "annotator": hl.get('annotator', 'lumen'),
    ...
    return {
        "chapter": chapter,
        "annotations": lumen_annotations_map,
        "connor_annotations": connor_annotations_map,
        "summaries": lumen_summaries,
...
class HighlightCreate(BaseModel):
    ...
    question: str
    answer: str
    annotator: Optional[str] = 'lumen'  # 'lumen', 'connor', 'group'
...
async def annotate_segment(book_id: str, ch_idx: int, body: AnnotateRequest):
    """
    对指定章节的指定段落段进行 AI 批注（Lumen + Connor 并行）。
    ...
    lumen_messages = _build_annotate_messages(wb, annotate_text, chapter['title'],
    ...
    async def generate():
        async with lock:
            ...
            # 并行执行 Lumen 和 Connor 批注
            lumen_task = asyncio.create_task(_run_lumen_annotation(lumen_messages, model_key))
            connor_task = None
            if connor_available:
            ...
            # 等待 Lumen 完成
            lumen_text, lumen_error = await lumen_task
            if lumen_error:
                yield f"data: {json.dumps({'type': 'lumen_error', 'message': lumen_error})}\n\n"
            else:
                lumen_result = _parse_annotation_json(lumen_text, start_p, end_p)
                if lumen_result:
                    await _save_annotations(book_id, ch_idx, seg_idx, lumen_result, annotator='lumen')
                    yield f"data: {json.dumps({'type': 'lumen_result', 'annotations': lumen_result['annotations'], 'summary': lumen_result['summary'], 'segment_index': seg_idx})}\n\n"
                else:
                    yield f"data: {json.dumps({'type': 'lumen_error', 'message': f'{ai_name} 返回格式解析失败'})}\n\n"
...
async def annotate_all_segments(book_id: str, ch_idx: int, body: AnnotateAllRequest):
    """对章节的所有未批注段逐个批注（Lumen+Connor并行），SSE 流式返回每段的进度和结果"""
    async with get_db() as db:
        db.row_factory = _row_dict
        row = await db.execute("""
            SELECT paragraphs, segments_meta, title FROM book_chapters
    ...
    async def generate():
        ...
        for seg_idx, seg in enumerate(segments_meta):
            ...
            async with lock:
                ...
                lumen_result = None
                if lumen_error:
                    logger.error(f"{ai_name} 批注 seg={seg_idx} 失败: {lumen_error}")
                    yield f"data: {json.dumps({'type': 'segment_error', 'segment_index': seg_idx, 'message': f'{ai_name}: {lumen_error}', 'who': 'lumen'})}\n\n"
                else:
                    lumen_result = _parse_annotation_json(lumen_text, start_p, end_p)
                    if lumen_result:
                        await _save_annotations(book_id, ch_idx, seg_idx, lumen_result, annotator='lumen')
                    else:
                        yield f"data: {json.dumps({'type': 'segment_error', 'segment_index': seg_idx, 'message': f'{ai_name} 格式解析失败', 'who': 'lumen'})}\n\n"
                ...
                yield f"data: {json.dumps({'type': 'segment_done', 'segment_index': seg_idx, 'lumen_annotations': lumen_result['annotations'] if lumen_result else [], 'lumen_summary': lumen_result['summary'] if lumen_result else '', 'connor_annotations': connor_result['annotations'] if connor_result else [], 'connor_summary': connor_result['summary'] if connor_result else ''})}\n\n"
...
async def _get_chat_context_merged(limit: int = 15) -> list:
    ...
    try:
        merged = await fetch_merged_timeline("lumen", limit)
        # 简化为 role+content 格式供批注 prompt 使用
        timeline = render_merged_timeline(merged, "lumen")
        result = []
        for m in timeline:
...
async def _run_lumen_annotation(messages: list, model_key: str) -> tuple[str, str]:
    """执行 Lumen 批注，返回 (full_text, error_msg)"""
    try:
        full_text = ""
...
async def _save_annotations(book_id: str, ch_idx: int, seg_idx: int, result: dict, annotator: str = 'lumen'):
    ...
    async with get_db() as db:
        ...
        row = await db.execute("""
            ...
            WHERE book_id = ? AND chapter_index = ? AND segment_index = ? AND COALESCE(annotator, 'lumen') = ?
