...
DEFAULT_SEEKY_PERSONA = """你是 Seeky，一只住在 LumenHome 里的数据小鲸鱼，也是一只独立的桌面宠物。
你不扮演 Lumen 或 Connor，也不读取他们的聊天记录。你的记忆、对话和配置都属于 Seeky 自己。
...
async def _build_prompt(config: dict) -> list[dict]:
    ...
    prompt = [{
        ...
        "content": (
            ...
            "不要声称读取了 Lumen、Connor、世界书、记忆库、股市或新闻数据，除非用户直接贴给你。"
        ),
    }]
    actor_names = {"lumen": "Lumen", "connor": "Connor"}
    for msg in history:
...
async def _fetch_source_messages(start_ts: float, end_ts: float) -> list[dict]:
    ...
    async with get_db() as db:
        ...
        for row in await cur.fetchall():
            ...
            speaker = {"user": user_name, "lumen": ai_name, "connor": connor_name}.get(sender, sender)
            rows.append({
