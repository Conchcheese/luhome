...
from chatroom import (
    ...
    get_chatroom_names,
    build_lumen_group_context, build_connor_group_context,
...
def _chatroom_auto_tts_voice(sender: str) -> str:
    if sender not in ("lumen", "connor"):
        return ""
    cfg = load_chatroom_config()
    if not cfg.get("tts_enabled"):
        return ""
    key = "tts_lumen_voice" if sender == "lumen" else "tts_connor_voice"
...
def _name_for_identity(identity: str) -> str:
    ...
    return {"user": user_name, "lumen": ai_name, "connor": connor_name}.get(identity, identity)


def _prefix_for_sender(sender: str) -> str:
    if sender in ("lumen", "connor"):
        return f"[{_name_for_identity(sender)}] "
    return ""


async def _save_main_memory_from_chatroom(room_id: str, msg_id: str, content: str) -> dict:
    """Save an Lumen-authored chatroom MEMORY command into the main Lumen memory table."""
    mem_now = time.time()
...
def _render_recent_room_messages_for_ai(msgs: list[dict]) -> list[dict]:
    ...
    这里不要把 Lumen/Connor 的历史发言渲染成 assistant 消息，否则模型在监控/动态等
    二次回复里容易把输入误当成多人剧本，继续输出 [Lumen]/[Connor] 前缀。
    """
    recent = []
    for m in msgs:
        sender = m.get("sender", "")
...
async def _get_or_create_lumen_private_conv() -> str:
    async with get_db() as db:
        db.row_factory = aiosqlite.Row
...
async def _send_lumen_private_whisper(content: str):
    content = content.strip()
    if not content:
        return

    conv_id = await _get_or_create_lumen_private_conv()
    now = time.time()
...
async def _get_or_create_connor_private_room() -> str:
    async with get_db() as db:
        ...
        await db.execute(
            "INSERT INTO chatroom_rooms (id, title, type, lumen_persona, connor_persona, created_at, updated_at) "
            "VALUES (?,?,?,?,?,?,?)",
...
async def _send_private_whisper(who_identity: str, content: str):
    if who_identity == "connor":
        ...
        await _send_lumen_private_whisper(content)
...
async def _process_chatroom_commands(full_text: str, room_id: str, who: str, msg_id: str, _q: asyncio.Queue) -> tuple[str, dict]:
    ...
    who: 内部身份 lumen 或 connor"""
    from ws import manager as ws_manager
    triggered = {}  # 收集需要后续处理的动作
    who_identity = "connor" if who.lower() == "connor" else "lumen"
    ...
    for match in MONITOR_CMD.finditer(full_text):
        try:
            ...
            pass
    _origin = "connor" if who.lower() == "connor" else "lumen"
        ...
        for mt_content, mt_reply in moment_matches:
            ...
            if mt_content:
                try:
                    ...
                    # who 是显示名，需要转为内部标识
                    author = "connor" if who.lower() == "connor" else "lumen"
            ...
            if mem_content:
                try:
                    ...
                    mr_data = {
                        ...
                        "content": mem_content,
                        "mem_id": mem_id,
                        "store": "main" if who_identity == "lumen" else "chatroom",
                    }
...
async def _chatroom_cam_check(room_id: str, sender: str, model_key: str, delay: float = 5.0):
    ...
    prefix_msgs = []
    if wb.get("ai_persona") and sender == "lumen":
...
async def _fetch_chatroom_source_rows_by_ids(source_ids: list[str]) -> list[dict]:
    rows = []
    user_name, ai_name, connor_name = get_chatroom_names()
    name_map = {"user": user_name, "lumen": ai_name, "connor": connor_name}
    async with get_db() as db:
        db.row_factory = aiosqlite.Row
        for source_id in source_ids:
            ...
            if row:
                rows.append({
                    ...
                    "role": "assistant" if row["sender"] in ("lumen", "connor") else "user",
...
async def update_config(body: ConfigUpdate):
    ...
    if body.tts_lumen_voice is not None:
        cfg["tts_lumen_voice"] = body.tts_lumen_voice
    if body.tts_connor_voice is not None:
        cfg["tts_connor_voice"] = body.tts_connor_voice
    if body.reply_order is not None and body.reply_order in ("lumen", "connor", "random", "manual"):
        cfg["reply_order"] = body.reply_order
    if body.connor_model is not None:
        cfg["connor_model"] = body.connor_model or "Codex"
    if body.lumen_model is not None:
        cfg["lumen_model"] = body.lumen_model
...
async def update_message_feedback(msg_id: str, body: MessageFeedbackUpdate):
    ...
    async with get_db() as db:
        ...
        can_rate = (
            row["room_type"] == "group" and row["sender"] in ("lumen", "connor")
        ) or (
...
async def send_message(room_id: str, body: MsgSend):
    ...
    if body.content:
        ...
        for t_recipient, t_amount_str in user_transfer_matches:
            try:
                t_val = float(t_amount_str)
                # 判断收款人：匹配 Lumen 名字则进 Lumen 钱包，否则默认 Connor 钱包
                recipient_stripped = (t_recipient or "").strip()
                is_lumen = recipient_stripped and recipient_stripped == ai_name
                if is_lumen:
                    # Lumen 钱包
                    async with get_db() as t_db:
                        t_now = time.time()
    ...
    if room_type == "group":
        # 群聊：两侧都更新为群聊
        manager.set_lumen_last_active(f"chatroom:{room_id}")
        manager.set_connor_last_active(room_id)
        # 用户在 Lumen 参与的群聊发消息时，也视为正在聊天，推迟哨兵巡逻。
    ...
    tts_lumen_voice = body.tts_lumen_voice
    ...
    async def _bg_generate():
        try:
            if room_type == "connor_1v1":
                ...
                # 群聊：Lumen 和 Connor 都回复
                await _generate_group_replies(room_id, room, msgs, model_key, connor_model_key, _q, context_limit,
                                              tts_enabled=tts_enabled, tts_lumen_voice=tts_lumen_voice, tts_connor_voice=tts_connor_voice,
                                              whisper_mode=whisper_mode)
        except Exception as e:
            import traceback
            traceback.print_exc()
...
async def reply_once(room_id: str, body: ReplyOnceTrigger):
    ...
    if speaker not in ("lumen", "connor"):
        return {"error": "speaker must be 'lumen' or 'connor'"}
        ...
        try:
            if speaker == "lumen":
                await _reply_lumen(
                    ...
                    tts_enabled=body.tts_enabled,
                    tts_voice=body.tts_lumen_voice,
                    whisper_mode=body.whisper_mode,
                )
            else:
                await _reply_connor(
...
async def regenerate_chatroom_message(msg_id: str, body: MsgRegenerate):
    ...
    async with get_db() as db:
        ...
        if not target:
            return {"error": "message not found"}
        if target["sender"] not in ("lumen", "connor"):
...
async def _generate_group_replies(room_id, room, msgs, model_key, connor_model_key, _q, context_limit, *, tts_enabled=False, tts_lumen_voice="", tts_connor_voice="", whisper_mode=False):
    """群聊回复：顺序执行，第二个 AI 能看到第一个的回复和工具执行结果"""
    query_text = msgs[-1]["content"] if msgs else ""

    lumen_first = random.choice([True, False])
    reply_order = load_chatroom_config().get("reply_order", "random")
    if reply_order == "lumen":
        lumen_first = True
    elif reply_order == "connor":
        lumen_first = False

    if lumen_first:
        digest = await _reply_lumen(room_id, msgs, context_limit, query_text, model_key, _q,
                                   tts_enabled=tts_enabled, tts_voice=tts_lumen_voice, whisper_mode=whisper_mode)
        ...
        _, updated_msgs = await _load_room_and_messages(room_id)
        await _reply_lumen(room_id, updated_msgs, context_limit, query_text, model_key, _q,
                          tts_enabled=tts_enabled, tts_voice=tts_lumen_voice, digest_result=digest, whisper_mode=whisper_mode)


async def _reply_lumen(room_id, msgs, context_limit, query_text, model_key, _q, *, tts_enabled=False, tts_voice="", digest_result=None, whisper_mode=False):
    ai_label = _name_for_identity("lumen")
    lumen_history, digest_out = await build_lumen_group_context(
        room_id, msgs, context_limit, query_text,
        digest_result=digest_result,
        whisper_mode=whisper_mode,
    )
    _process_voice_attachments(lumen_history)
    lumen_msg_id = f"cm_{int(time.time() * 1000)}_a"
    await _q.put({"type": "lumen_start", "id": lumen_msg_id})
    ...
    usage_meta: dict = {}
    try:
        async for chunk in stream_ai(lumen_history, model_key, usage_meta):
            if chunk.startswith(CLI_STATUS_PREFIX):
                await _q.put({"type": "lumen_status", "text": chunk[len(CLI_STATUS_PREFIX):]})
                continue
            full_text += chunk
            await _q.put({"type": "lumen_chunk", "content": chunk})
    except Exception as e:
        has_error = True
        error_text = str(e)
        full_text += f"\n[{ai_label} 回复出错: {e}]"
        await _q.put({"type": "lumen_chunk", "content": f"\n[回复出错: {e}]"})

    # 工具指令处理（从文本中剥离并执行）
    try:
        clean_text, triggered = await _process_chatroom_commands(full_text, room_id, "lumen", lumen_msg_id, _q)
    except Exception as e:
        import traceback
        traceback.print_exc()
        print(f"[CHATROOM] _process_chatroom_commands 异常: {e}")
    ...
    if tts_enabled and tts_voice and clean_text:
        tts = TTSStreamer(lumen_msg_id, tts_voice, manager, sse_queue=_q)
        tts.feed(clean_text)
        await tts.flush()

    # 保存干净文本
    saved_imgs = await _extract_and_save_images(clean_text)
    lumen_msg = await _save_msg(
        room_id, "lumen", clean_text, lumen_msg_id,
    ...
    await _q.put({"type": "lumen_done", "message": lumen_msg})
    await _emit_chatroom_debug(_q, _chatroom_debug_payload(
        room_id=room_id,
        model_key=model_key,
        msg_id=lumen_msg_id,
    ...
    _fire_chatroom_followups(triggered, room_id, "lumen", model_key)
        ...
        try:
            ...
            for round_num in range(max_rounds):
                ...
                if reply_order == "connor":
                    lumen_first = False
                elif reply_order == "lumen":
                    lumen_first = True
                else:
                    lumen_first = random.choice([True, False])

                if lumen_first:
                    digest = await _reply_lumen(
                        room_id, msgs, context_limit, query_text, model_key, _q,
                        tts_enabled=tts_enabled, tts_voice=tts_lumen_voice, digest_result=digest,
                    )
...
async def get_memory_source_legacy(mem_id: str):
    ...
    for r in rows:
        messages.append({
            "role": "assistant" if r["sender"] in ("lumen", "connor") else "user",
