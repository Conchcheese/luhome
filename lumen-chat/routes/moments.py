...
def _author_display(author: str) -> str:
    ...
    return {"user": user_name, "lumen": ai_name, "connor": connor_name}.get(author, author)
    ...
    async with get_db() as db:
        ...
        if who == "lumen":
            ...
            for row in await cur.fetchall():
                sender = "lumen" if row["sender"] == "assistant" else "user"
                messages.append({
...
async def add_comment(moment_id: str, body: CommentCreate):
    ...
    async with get_db() as db:
        ...
        if body.reply_to_id:
            ...
            if parent_comment["author"] in ("lumen", "connor"):
                target_ai_author = parent_comment["author"]
        elif moment_row["author"] in ("lumen", "connor"):
