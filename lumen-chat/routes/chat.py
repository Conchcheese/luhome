...
async def edit_resend_message(msg_id: str, body: MsgEditResend):
    ...
    # Lumen 侧：用户在 Lumen 私聊发消息
    manager.set_lumen_last_active("private")
    ...
    # ── 合并私聊 + 群聊消息为统一时间线 ──
    merged = await fetch_merged_timeline("lumen", body.context_limit, conv_id=conv_id)
                    ...
                    if mt_content:
                        ...
                        async with get_db() as mt_db:
                            await mt_db.execute(
                                ...
                                (mt_id, "lumen", mt_content, conv_id, ai_msg_id, expect, mt_now)
                            )
                            await mt_db.commit()
                        mt_data = {"type": "moment_new", "data": {
                            "id": mt_id, "author": "lumen", "content": mt_content,
                        ...
                        if expect:
                            ...
                            asyncio.create_task(_trigger_ai_replies(mt_id, exclude_author="lumen"))
...
async def regenerate_message(conv_id: str, context_limit: int = 30, whisper_mode: bool = False, fast_mode: bool = False, temperature: Optional[float] = None, max_tokens: Optional[int] = None, tts_enabled: bool = False, tts_voice: str = ""):
    ...
    # ── 合并私聊 + 群聊消息为统一时间线 ──
    merged = await fetch_merged_timeline("lumen", context_limit, conv_id=conv_id)
    history = render_merged_timeline(merged, "lumen")
