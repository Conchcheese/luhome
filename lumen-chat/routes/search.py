...
async def search_messages(q: str = Query(..., min_length=1, max_length=80), limit: int = Query(60, ge=1, le=120)):
    ...
    wb = load_worldbook()
    user_name, lumen_name, connor_name = get_chatroom_names()
    user_name = user_name or wb.get("user_name") or "你"
    lumen_name = lumen_name or wb.get("ai_name") or "AI"
    connor_name = connor_name or "Connor"
    name_map = {
        "user": user_name,
        "assistant": lumen_name,
        "lumen": lumen_name,
        "connor": connor_name,
        "system": "系统",
    }
    ...
    for row in main_rows:
        items.append({
            "source": "lumen_private",
            "source_label": f"{lumen_name}私聊",
            "id": row["id"],
            "target_id": row["target_id"],
            "target_title": row["target_title"] or f"{lumen_name}私聊",
    ...
    return {
        "items": items[:limit],
        "query": keyword,
        "names": {"user": user_name, "lumen": lumen_name, "connor": connor_name},
    }
