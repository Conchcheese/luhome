...
from chatroom import build_lumen_group_context, build_connor_group_context, get_chatroom_names, load_chatroom_config, stream_connor_cli
...
PLAYERS = ["user", "lumen", "connor"]
DEFAULT_PLAYER_NAMES = {"user": "用户", "lumen": "AI", "connor": "Connor"}
...
def _player_names() -> dict[str, str]:
    ...
    return {"user": user_name, "lumen": ai_name, "connor": connor_name}
...
async def _speak_ai_speech(game_id: str, player: str, speech: str, event_index: int):
    ...
    if player not in ("lumen", "connor") or not speech:
        return
    cfg = load_chatroom_config()
    if not cfg.get("tts_enabled"):
        return
    voice = cfg.get("tts_lumen_voice" if player == "lumen" else "tts_connor_voice", "")
    if not voice:
        return
    try:
...
def _game_prompt(state: dict, player: str, moves: Optional[list[dict]] = None) -> str:
    ...
    return (
        "[斗地主牌局任务]\n"
        f"你正在和 {_player_name('user')}、{_player_name('lumen')}、{_player_name('connor')} 玩一局真实斗地主。你必须保持原本人设和说话风格，但本次回复只允许输出一个 JSON 对象。\n"
...
async def _ask_ai(state: dict, player: str, moves: Optional[list[dict]] = None) -> dict:
    ...
    query = "斗地主牌局 JSON 决策"
    if player == "lumen":
        messages, _ = await build_lumen_group_context(
...
async def ai_step():
    async with _state_lock:
        ...
        if state.get("phase") == "game_over":
            return {"ok": True, "state": _public_state(state)}
        if state.get("current_player") not in ("lumen", "connor"):
