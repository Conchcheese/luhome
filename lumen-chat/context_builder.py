"""
共享上下文构建模块：为 Lumen / Connor 构建完整的系统能力、记忆、时间感知注入。
...
def format_ability_block(abilities: list[str]) -> str:
    block = (
        "[系统能力]\n"
        "以下方括号指令是 LumenHome 本地动作协议，不是普通文本装饰。"
...
async def build_memory_blocks(
    ...
    *,
    ...
    参数:
      ...
      use_main_memories: 是否使用 Lumen 主记忆库
...
async def fetch_merged_timeline(
    ...
    *,
    ...
    Args:
        who: "lumen" — 看到 Lumen 私聊 + 群聊；"connor" — 看到 Connor 1v1 + 群聊
        limit: 返回的最大消息总数
        conv_id: Lumen 私聊的 conv_id（可选，为 None 时自动取最近会话）
...
def render_merged_timeline(
    ...
    - who: "lumen" — Lumen 视角；"connor" — Connor 视角。
    - 历史消息统一渲染为 user 角色里的“历史消息 - 说话人：内容”，不再把历史里的
      Lumen/Connor 渲染成 assistant。这样可以避免模型把群聊记录误当成需要续写的多人剧本。
