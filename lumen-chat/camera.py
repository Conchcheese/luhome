...
async def async_get_last_lumen_timeline_user_msg_time(conv_id: str = None) -> float:
    """Lumen 视角的最后用户发言时间：合并私聊 + 最近群聊。"""
    latest_ts = 0.0
    async with get_db() as db:
...
async def async_get_recent_lumen_timeline_text(
    conv_id: str = None,
    limit: int = 10,
    *,
    user_name: str = "用户",
    ...
    async with get_db() as db:
        ...
        if room:
            cur = await db.execute(
                ...
                "WHERE room_id=? AND sender IN ('user','lumen','connor') "
                "ORDER BY created_at DESC LIMIT ?",
                (room["id"], limit),
            )
            rows.extend(dict(r) for r in await cur.fetchall())
    ...
    for r in rows:
        ...
        if sender == "user":
            name = user_name
        elif sender in ("assistant", "lumen"):
            name = ai_name
        elif sender == "connor":
            name = connor_name
        else:
...
def _strip_leading_cli_role_header(text: str) -> str:
    ...
    if not text:
        return text
    return re.sub(r"^\s*\[(?:Assistant|Model|AI|Lumen)\]\s*", "", text, count=1).lstrip()
...
class CameraMonitor:
    ...
    async def _analyze_and_log(self, screenshot_filename: str):
        ...
        last_user_ts = await async_get_last_lumen_timeline_user_msg_time(conv_id)
        ...
        recent_chat_text = ""
        try:
            recent_chat_text = await async_get_recent_lumen_timeline_text(
                conv_id,
                10,
                user_name=user_name,
                ai_name=ai_name,
...
{log_history if log_history else "（暂无历史日志）"}
    ...
    async def _call_core(self, trigger_log: str, last_user_ts: float, summary: str = "", core_reason: str = "", cached_logs: list = None, screenshot_filename: str = ""):
        ...
        target = schedule_mgr._resolve_target({"origin": "lumen"})
        is_chatroom = target["type"] == "chatroom"
        if is_chatroom:
            try:
                from chatroom import load_chatroom_config
                chatroom_model = (load_chatroom_config().get("lumen_model") or "").strip()
                if chatroom_model:
        ...
        if is_chatroom:
            await schedule_mgr._save_to_chatroom(
                target["room_id"], "lumen", sys_content, full_text, core_msg_id, "[]", []
            )
        else:
            now = time.time()
...
async def perform_cam_check(conv_id: str, model_key: str):
    ...
    from context_builder import fetch_merged_timeline, render_merged_timeline
    merged = await fetch_merged_timeline("lumen", 6, conv_id=conv_id)
    recent = render_merged_timeline(merged, "lumen")
