...
async def _auto_digest_loop():
    """每 30 分钟检查一次，若用户已 30 分钟未发消息（Lumen 私聊+群聊）则自动总结"""
    import aiosqlite, time as _time
    while True:
        await asyncio.sleep(30 * 60)  # 30 分钟
        try:
            # 检查最后一条用户消息的时间（Lumen 私聊 + 群聊取最新的）
            latest_ts = 0
...
app.mount("/lumen-pet", StaticFiles(directory=str(BASE_DIR.parent / "LumenPet")), name="lumen-pet")
