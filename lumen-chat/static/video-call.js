...
const videoCall = (() => {
  ...
  async function _startCamera() {
    ...
    if (window.LumenCamera) {
      ...
      const ok = window.LumenCamera.start(facing);
      if (ok) {
  ...
  function _pollNativeFrame() {
    if (!_useNativeCamera || !window.LumenCamera) return;
  ...
  function _stopCamera() {
    ...
    if (_useNativeCamera && window.LumenCamera) {
      window.LumenCamera.stop();
  ...
  function _startAudioStream() {
    ...
    if (window.LumenAudio) {
      if (window.LumenAudio.isRecording()) {
  ...
  function _stopAudioStream() {
    if (_useNativeAudio && window.LumenAudio && _ownNativeBridge) {
  ...
  function _startRecord() {
    ...
    if (_useNativeCamera && window.LumenVideo) {
      // ── Android 原生录制 ──
      // LumenVideo 复用 CameraBridge + AudioBridge 的帧
      const w = window.LumenCamera ? window.LumenCamera.getRotatedWidth() : 480;
      const h = window.LumenCamera ? window.LumenCamera.getRotatedHeight() : 640;
      const ok = window.LumenVideo.startRecord(w, h);
      if (!ok) {
        console.error('[VideoCall] LumenVideo.startRecord failed');
        _cancelRecord();
        return;
      }
  ...
  async function _stopRecord() {
    ...
    try {
      ...
      if (_useNativeCamera && window.LumenVideo) {
        ...
        const b64 = window.LumenVideo.stopRecord();
  ...
  function _hangup() {
    ...
    if (_videoRecording) {
      _videoRecording = false;
      _clearRecordUI();
      if (_useNativeCamera && window.LumenVideo) window.LumenVideo.cancel();
      if (_videoRecorder && _videoRecorder.state !== 'inactive') _videoRecorder.stop();
      if (_audioForASR && _audioForASR.state !== 'inactive') _audioForASR.stop();
      _videoRecorder = null;
      _audioForASR = null;
  ...
  // ── 暴露给 Android 原生桥接的方法 ──
  const _origChunkHandler = window.onLumenAudioChunk;
  window.onLumenAudioChunk = function(b64) {
    if (_active) {
      _onNativeChunk(b64);
