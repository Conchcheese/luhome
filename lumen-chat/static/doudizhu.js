...
async function loadPlayerNames() {
  try {
    ...
    playerNames.lumen = cfg.ai_name || playerNames.lumen;
    playerNames.connor = cfg.connor_name || playerNames.connor;
  } catch {}
}

const playerAvatars = {
  lumen: "/public/gropicon1.png",
...
function opponentOrder() {
  const order = gameState.order?.length ? gameState.order : ["user", "lumen", "connor"];
...
function renderSeats() {
  const [left = "lumen", right = "connor"] = opponentOrder();
...
async function announceSettlement() {
  ...
  $("settlementStatus").textContent = `已发到最新群聊，${playerNames.lumen} 和 ${playerNames.connor} 会接着回应。`;
  if (data.state) setState(data.state);
}
...
function scheduleAiIfNeeded() {
  ...
  if (!["lumen", "connor"].includes(gameState.currentPlayer)) return;
  aiTimer = setTimeout(runAiLoop, 450);
}

async function runAiLoop() {
  if (!gameState || aiBusy || !["lumen", "connor"].includes(gameState.currentPlayer)) return;
  aiBusy = true;
  renderControls();
  try {
    while (gameState && ["lumen", "connor"].includes(gameState.currentPlayer) && gameState.phase !== "game_over") {
