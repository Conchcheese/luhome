...
const _clientId = localStorage.getItem('lumen_client_id') || (() => {
  const a = new Uint8Array(16); crypto.getRandomValues(a);
  a[6] = (a[6] & 0x0f) | 0x40; a[8] = (a[8] & 0x3f) | 0x80;
  const h = [...a].map(b => b.toString(16).padStart(2, '0')).join('');
  const id = `${h.slice(0,8)}-${h.slice(8,12)}-${h.slice(12,16)}-${h.slice(16,20)}-${h.slice(20)}`;
  localStorage.setItem('lumen_client_id', id); return id;
})();
let ws = null;
...
function applyLumenTheme(theme) {
  const next = theme === 'light' ? 'light' : 'dark';
  document.documentElement.dataset.theme = next;
  document.body.dataset.theme = next;
  localStorage.setItem('lumen_chat_theme', next);
  const meta = document.querySelector('meta[name="theme-color"]');
  if (meta) meta.setAttribute('content', next === 'dark' ? '#050923' : '#eef3ff');
  // 通知原生 App 切换状态栏图标颜色
  if (window.LumenStatusBar) window.LumenStatusBar.setBarStyle(next);
}

function toggleLumenTheme() {
  applyLumenTheme(document.body.dataset.theme === 'light' ? 'dark' : 'light');
}

applyLumenTheme(localStorage.getItem('lumen_chat_theme') || 'dark');
window.addEventListener('storage', e => {
  if (e.key === 'lumen_chat_theme') applyLumenTheme(e.newValue || 'dark');
});
...
async function init() {
  ...
  const lastId = localStorage.getItem('lumen_last_conv');
  ...
  // 恢复上下文数量设置
  const savedCtx = localStorage.getItem('lumen_context_limit');
  if (savedCtx) { $("contextSlider").value = savedCtx; $("contextValue").textContent = savedCtx + '条'; }
  // 恢复温度设置
  const savedTemp = localStorage.getItem('lumen_temperature');
  if (savedTemp) { $("tempSlider").value = savedTemp; $("tempValue").textContent = savedTemp; }
  // 恢复最大回复长度设置
  const savedMaxTokens = localStorage.getItem('lumen_max_tokens');
...
function onMicSourceChange() {
  ...
  localStorage.setItem('lumen_voice_mic_source', newSrc);
...
async function toggleVoice() {
  ...
  localStorage.setItem('lumen_voice_enabled', enabled);
  localStorage.setItem('lumen_voice_wakeword', wakeWord);
...
function updateVoiceWakeWord() {
  ...
  localStorage.setItem('lumen_voice_wakeword', ww);
...
async function playWakeupReply() {
  // 播放唤醒应答音频
  try {
    const audio = new Audio('/public/LumenResponse.mp3');
...
const remoteVoice = {
  ...
  async start(wakeWord) {
    ...
    if (window.LumenAudio) {
      const ok = window.LumenAudio.start();
      if (ok) {
  ...
  stop() {
    ...
    voiceEnabled = false;
    voiceInCall = false;
    if (this._useNative && window.LumenAudio) {
      window.LumenAudio.stop();
      this._useNative = false;
    }
...
// 初始化语音设置
(function initVoice() {
  const ww = localStorage.getItem('lumen_voice_wakeword') || '老公';
  $('voiceWakeWord').value = ww;
  // 自动检测：App 内或移动端默认用手机麦克风
  const ua = navigator.userAgent;
  const isApp = ua.includes('LumenChatApp');
  const isMobile = /Android|iPhone|iPad/i.test(ua);
  const savedSrc = localStorage.getItem('lumen_voice_mic_source');
  if (savedSrc) {
...
let ttsEnabled = localStorage.getItem('lumen_tts_enabled') === 'true';
let ttsVoiceId = localStorage.getItem('lumen_tts_voice') || '';
...
function toggleTTS() {
  ...
  localStorage.setItem('lumen_tts_enabled', ttsEnabled);
  _sendTTSState();
  if (!ttsEnabled) {
    ttsManualStop = true;
    clearTTSResumeTimer();
...
function changeTTSVoice() {
  ...
  localStorage.setItem('lumen_tts_voice', ttsVoiceId);
  _sendTTSState();
}
...
function handleSync(msg) {
  ...
  if (type === "conv_created") {
    ...
    renderConvList();
    if (data.id === currentConvId) { currentConvId = null; currentMessages = []; localStorage.removeItem('lumen_last_conv'); renderMessages(); }
...
function renderMessages() {
  ...
  el.innerHTML = loadMoreBtn + messagesForDisplay(currentMessages).map(m => {
    ...
    const starBadge = m.starred ? '<span class="msg-star-badge"><svg viewBox="0 0 24 14" width="22" height="13" xmlns="http://www.w3.org/2000/svg" style="display:inline-block;vertical-align:middle;image-rendering:pixelated"><rect x="4" y="0" width="16" height="2" fill="#D97757"/><rect x="0" y="2" width="24" height="2" fill="#D97757"/><rect x="0" y="4" width="8" height="2" fill="#D97757"/><rect x="8" y="4" width="2" height="2" fill="#1E1008"/><rect x="10" y="4" width="4" height="2" fill="#D97757"/><rect x="14" y="4" width="2" height="2" fill="#1E1008"/><rect x="16" y="4" width="8" height="2" fill="#D97757"/><rect x="4" y="6" width="4" height="2" fill="#D97757"/><rect x="8" y="6" width="2" height="2" fill="#1E1008"/><rect x="10" y="6" width="4" height="2" fill="#D97757"/><rect x="14" y="6" width="2" height="2" fill="#1E1008"/><rect x="16" y="6" width="4" height="2" fill="#D97757"/><rect x="4" y="8" width="16" height="2" fill="#D97757"/><rect x="4" y="10" width="2" height="4" fill="#D97757"/><rect x="8" y="10" width="2" height="4" fill="#D97757"/><rect x="14" y="10" width="2" height="4" fill="#D97757"/><rect x="18" y="10" width="2" height="4" fill="#D97757"/></svg></span>' : '';
...
window.jumpToGlobalSearchTarget = async function(target) {
  if (!target) return;
  if (target.source === 'lumen_private') {
    closeSubPage(true);
...
async function selectConv(id) {
  currentConvId = id;
  localStorage.setItem('lumen_last_conv', id);
  msgDebugData = {};
...
function saveImage(url) {
  fetch(url)
    ...
    .then(blob => {
      ...
      if (window.LumenImageSaver) {
        const reader = new FileReader();
        reader.onloadend = () => {
          const base64 = reader.result.split(',')[1];
          const filename = url.split('/').pop() || 'image.png';
          window.LumenImageSaver.save(base64, filename);
        };
...
async function startCam() {
  ...
  if (window.LumenCamera) {
    const ok = window.LumenCamera.start(_camFacing === 'user' ? 'user' : 'environment');
    if (ok) {
      _camUseNative = true;
      const vid = document.getElementById('camVideo');
      const img = document.getElementById('camImg');
...
function pollCamFrame() {
  if (!_camUseNative || !window.LumenCamera) return;
  const frame = window.LumenCamera.getFrame();
  if (frame) {
    const img = document.getElementById('camImg');
...
function stopCam() {
  ...
  if (_camUseNative && window.LumenCamera) { window.LumenCamera.stop(); _camUseNative = false; }
...
async function flipCam() {
  ...
  if (_camUseNative && window.LumenCamera) {
    window.LumenCamera.flip();
    const img = document.getElementById('camImg');
...
async function capturePhoto() {
  let dataUrl = null;
  if (_camUseNative && window.LumenCamera) {
    const b64 = window.LumenCamera.capture();
...
async function _voiceStopRecord(evt) {
  ...
  if (_voiceUseNative) {
    ...
    if (window.LumenAudio) window.LumenAudio.stop();
...
function _voiceCleanup() {
  ...
  if (_voiceUseNative && window.LumenAudio) { try { window.LumenAudio.stop(); } catch {} }
  _voiceRecording = false;
  _voiceChunks = [];
  _voiceNativeChunks = [];
  const btn = $('voiceHoldBtn');
...
async function toySendData2(hexCmd) {
  ...
  if (window.LumenBle && window.LumenBle.isConnected()) {
    toyLog('→ ' + hexCmd, 'wl-send');
    window.LumenBle.sendData(hexCmd);
    return;
  }
  // Web Bluetooth（浏览器）
...
async function toyToggleConnect() {
  ...
  // 原生 BLE 桥接（Android APK）
  if (window.LumenBle) { window.LumenBle.connect(); return; }
  // Web Bluetooth（浏览器）
...
function openWhisper() {
  closeSidebar();
  // 检查原生 BLE 桥接的实际连接状态
  if (window.LumenBle && typeof window.LumenBle.isConnected === 'function') {
    toyConnected = window.LumenBle.isConnected();
  }
  toyLoadPresets();
  toyRenderGrid();
  toyUpdateUI();
...
const _GIFT_KNOWN_KEY = 'lumen_gift_known_ids';
...
function getSubPageThemeChromeColors() {
  const theme = localStorage.getItem('lumen_chat_theme') || document.body.dataset.theme || 'dark';
  return theme === 'light'
...
function applySubPageThemeChrome() {
  ...
  if (window.LumenStatusBar) window.LumenStatusBar.setBarStyle(colors.style);
}
...
function syncSubPageChromeFromFrame() {
  ...
  if (window.LumenStatusBar) window.LumenStatusBar.setBarStyle(isSubPageLightColor(safeColor) ? 'light' : 'dark');
}
function syncSubPageMode(url) {
  ...
  if (window.LumenStatusBar) {
    if (isHome) applyLumenTheme(localStorage.getItem('lumen_chat_theme') || document.body.dataset.theme || 'dark');
    else window.LumenStatusBar.setBarStyle(getSubPageThemeChromeColors().style);
  }
}
...
function closeSubPage(skipReload = false) {
  ...
  currentSubPage = null;
  applyLumenTheme(localStorage.getItem('lumen_chat_theme') || document.body.dataset.theme || 'dark');
...
// ── 深空星空 ──
function drawStarfield(canvasId) {
  ...
  for (let i = 0; i < 220; i++) {
    const x = Math.random() * W;
    const y = Math.random() * H;
    const r = Math.random() * 1.0 + 0.55;
    const a = Math.random() * 0.40 + 0.40;
    ctx.beginPath();
    ctx.arc(x, y, r, 0, Math.PI * 2);
    ctx.fillStyle = `rgba(225, 242, 255, ${a})`;
    ctx.fill();
  }
  // 亮星（带光晕）
  for (let i = 0; i < 60; i++) {
    const x = Math.random() * W;
    const y = Math.random() * H;
    const r = Math.random() * 1.4 + 1.0;
    const a = Math.random() * 0.30 + 0.60;
    ctx.beginPath();
    ctx.arc(x, y, r, 0, Math.PI * 2);
    ctx.fillStyle = `rgba(235, 248, 255, ${a})`;
    ctx.fill();
    const grd = ctx.createRadialGradient(x, y, 0, x, y, r * 6);
    grd.addColorStop(0, `rgba(180, 225, 255, ${a * 0.50})`);
  ...
  for (let i = 0; i < 12; i++) {
    const x = Math.random() * W;
    const y = Math.random() * H;
    const r = Math.random() * 1.0 + 2.0;
    const a = Math.random() * 0.20 + 0.75;
    ctx.beginPath();
    ctx.arc(x, y, r, 0, Math.PI * 2);
    ctx.fillStyle = `rgba(245, 252, 255, ${a})`;
    ctx.fill();
    const grd = ctx.createRadialGradient(x, y, 0, x, y, r * 8);
    grd.addColorStop(0, `rgba(200, 235, 255, ${a * 0.55})`);
    grd.addColorStop(1, 'transparent');
    ctx.beginPath();
...
drawStarfield('chatStarsCanvas');
window.addEventListener('resize', () => drawStarfield('chatStarsCanvas'));

// ── 白天代码雨 ──
(function() {
  const canvas = document.getElementById('chatRainCanvas');
  if (!canvas) return;
  const ctx = canvas.getContext('2d');
  const BG = '#fff4ed';
  const CHARS = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdef@#$%&<>{}[]|\\+-=~*';
  const COL_W = 16, FS = 13, INTERVAL = 45;
  ...
  new MutationObserver(() => { if (document.body.dataset.theme !== 'dark') resize(); })
  ...
  function draw(ts) {
    ...
    ctx.fillStyle = 'rgba(255,244,237,0.072)';
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    ctx.font = `${FS}px "Courier New", monospace`;
    for (let i = 0; i < cols; i++) {
      const ch = CHARS[Math.floor(Math.random() * CHARS.length)];
      const y  = drops[i] * FS;
      const a  = 0.50 + Math.random() * 0.32;
      ctx.fillStyle = `rgba(217,119,87,${a})`;
      ctx.fillText(ch, i * COL_W + 2, y);
      if (y > canvas.height && Math.random() > 0.975) drops[i] = -(Math.random() * 15);
      drops[i] += speeds[i];
    }
  }
  requestAnimationFrame(draw);
