...
(function () {
  const KEY = 'lumen_chat_theme';
  const STYLE_ID = 'lumen-theme-css';
  ...
  function updateThemeChrome(theme) {
    ...
    if (window.LumenStatusBar) window.LumenStatusBar.setBarStyle(theme);
  }

  function applyLumenTheme(theme, options) {
    ...
    window.dispatchEvent(new CustomEvent('lumen-theme-applied', { detail: { theme: next } }));
    return next;
  }
  ...
  window.LumenTheme = Object.assign(window.LumenTheme || {}, {
    key: KEY,
    apply: applyLumenTheme,
  ...
  window.applyLumenTheme = applyLumenTheme;

  function initTheme() {
    ensureThemeStyles();
    applyLumenTheme(initialTheme(), { persist: Boolean(localStorage.getItem(KEY)) });
  }
  ...
  window.addEventListener('storage', event => {
    if (event.key === KEY) applyLumenTheme(event.newValue || initialTheme(), { persist: false });
  });
})();
...
let _giftKnownIds = new Set(JSON.parse(localStorage.getItem('lumen_gift_known_ids') || '[]'));
...
function _rememberGiftSeen(giftId) {
  ...
  localStorage.setItem('lumen_gift_known_ids', JSON.stringify([..._giftKnownIds].slice(-200)));
}
