...
function normalizeSeekyMessageRole(msg) {
  ...
  if (/^\[(lumen|lumens?)[^\]]*\]/i.test(content)) return 'lumen';
...
function renderMainChatItem(item) {
  ...
  const isUser = item.role === 'user';
  const isActor = item.role === 'lumen' || item.role === 'connor';
...
function renderHistoryMessage(message) {
  ...
  const actorCls = isUser ? 'user' : `assistant ${role === 'lumen' || role === 'connor' ? 'actor' : ''}`;
  return `
