...
class ConnectionManager:
    def __init__(self):
        ...
        # ── 各侧用户最后活跃窗口追踪 ──
        # "private" = Lumen 私聊, "chatroom:<room_id>" = 群聊/Connor 私聊
        self._lumen_last_active: str = "private"        # Lumen 侧：用户最后在 Lumen 私聊 or 群聊
    ...
    def set_lumen_last_active(self, target: str):
        """设置 Lumen 侧用户最后活跃窗口。target: 'private' 或 'chatroom:<room_id>'"""
        self._lumen_last_active = target

    def set_connor_last_active(self, room_id: str):
        """设置 Connor 侧用户最后活跃窗口（群聊或 Connor 私聊的 room_id）"""
        self._connor_last_active = room_id

    def get_lumen_last_active(self) -> str:
        return self._lumen_last_active
