...
def get_schedule_origin_name(origin: str | None) -> str:
    ...
    wb = load_worldbook()
    origin = (origin or "lumen").strip().lower()
    if origin == "connor":
        try:
            from chatroom import load_chatroom_config
            cfg = load_chatroom_config()
...
class ScheduleManager:
    ...
    async def _fire_alarm(self, item: dict):
        ...
        sender = "connor" if origin == "connor" else "lumen"
        ...
        # 获取 conv_id（始终需要，用于 Lumen 的上下文获取和保存）
        async with get_db() as db:
            db.row_factory = aiosqlite.Row
        ...
        if origin == "connor" and is_chatroom:
            ...
            merged = await fetch_merged_timeline("lumen", 20, conv_id=conv_id)
            history = render_merged_timeline(merged, "lumen")
        ...
        if origin == "connor":
            ...
            # Lumen 来源用常规 stream_ai
            alarm_tts = None
            tts_voice = _tts_voice_for_target(is_chatroom, "lumen")
            if tts_voice:
                alarm_tts = TTSStreamer(ai_msg_id, tts_voice, manager)
...
# ── 指令解析（在 AI 回复完成后调用） ──────────────
async def process_schedule_commands(full_text: str, conv_id: str = None, origin: str = "lumen", origin_room_id: str = "", after_msg_id: str = None) -> str:
    """
    检测并处理 AI 回复中的日程指令，返回 strip 后的文本。
    即使 AI 格式有误也不抛异常，静默跳过。
    origin: 'lumen' 或 'connor'，标记创建者
    origin_room_id: 群聊/Connor私聊的 room_id（空=Lumen私聊创建）
...
async def _add_schedule(stype: str, trigger_at: str, content: str, origin: str = "lumen", origin_room_id: str = ""):
