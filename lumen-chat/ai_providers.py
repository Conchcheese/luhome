...
_LEADING_CLI_ROLE_HEADER_RE = re.compile(r'^\s*\[(?:Assistant|Model|AI|Lumen)\]\s*', re.IGNORECASE)
...
def _summarize_antigravity_log(log_path: Path | None) -> str:
    ...
    if "You are not logged into Antigravity" in text and not auth_succeeded:
        return "Antigravity CLI 还没有完成登录。请先在终端运行 agy，选 Google OAuth 完成 CLI 登录，再回到 LumenHome 重试。"
