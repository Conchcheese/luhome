...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        for col, defn in [
            ("origin", "TEXT DEFAULT 'lumen'"),        # 'lumen' | 'connor'
            ("origin_room_id", "TEXT DEFAULT ''"),     # 创建时所在的群聊/Connor私聊 room_id（空=Lumen私聊）
        ]:
            try:
        ...
        # 迁移：为 book_annotations 添加 annotator 字段（lumen/connor）
        try:
            await db.execute("ALTER TABLE book_annotations ADD COLUMN annotator TEXT DEFAULT 'lumen'")
        except:
            pass
                ...
                if 'UNIQUE(book_id,chapter_index,segment_index)' in create_sql.replace(' ', ''):
                    ...
                    await db.execute("""
                        ...
                        SELECT id, book_id, chapter_index, segment_index,
                            annotations, summary, created_at, updated_at, COALESCE(annotator, 'lumen')
        ...
        try:
            await db.execute("ALTER TABLE book_highlights ADD COLUMN annotator TEXT DEFAULT 'lumen'")
        except:
            pass
        try:
        ...
        # gifts 表新增字段（向后兼容迁移）
        try:
            await db.execute("ALTER TABLE gifts ADD COLUMN sender TEXT DEFAULT 'lumen'")
        except:
            pass
        # ── 基金持仓表 ──
        await db.execute("""
        ...
        try:
            await db.execute("ALTER TABLE chatroom_rooms RENAME COLUMN aion_persona TO lumen_persona")
        except:
            pass
        # ── 聊天室消息表 ──
        await db.execute("""
