...
async def _notify_sentinel(old_state: str, new_state: str, status: dict, event_desc: str):
    ...
    from camera import (
        read_logs_since,
        append_monitor_log,
        async_get_last_lumen_timeline_user_msg_time,
        async_get_recent_lumen_timeline_text,
    )
    ...
    last_user_ts = await async_get_last_lumen_timeline_user_msg_time()
    last_user_time_str = (
...
async def _call_core_location(
    ...
    """唤醒 Core 通知位置变化"""
    from camera import read_logs_since, append_monitor_log, async_get_last_lumen_timeline_user_msg_time
