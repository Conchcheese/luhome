...
async def fetch_source_details(memories: list[dict], keywords: list[str]) -> str:
                    ...
                    if prefix == "private":
                        ...
                        if row:
                            ...
                            if key not in seen:
                                seen.add(key)
                                matched_rows.append({
                                    "role": "assistant" if row["sender"] == "lumen" else "user",
                                    "content": row["content"],
                                    "created_at": row["created_at"],
                                    "_sender": row["sender"],
                                })
        ...
        async with get_db() as db:
            ...
            if group_room:
                ...
                for gr in await cur.fetchall():
                    rows.append({"role": "assistant" if gr["sender"] == "lumen" else "user",
    ...
    for row in matched_rows:
        ...
        if sender:
            name = {"user": user_name, "lumen": ai_name, "connor": connor_name}.get(sender, sender)
        else:
...
async def _draft_main_daily_rows(rows: list[dict], model_key: str) -> dict:
    ...
    parsed, raw = await _call_daily_compress_model("lumen", prompt, model_key)
    if not parsed:
        return {
            "ok": False,
    ...
    if covered or created_daily or created_important:
        ...
        async with get_db() as db:
            await db.execute(
                ...
                "VALUES (?,?,?,?,?,?,?)",
                (
                    f"dmc_{time.time_ns()}", "lumen", json.dumps(sorted(covered), ensure_ascii=False),
